...
`Distribution.sol` 是一个接口，通过它 Solidity 合约可以与 Cosmos SDK 分发模块进行交互。
对于开发人员来说，这非常方便，因为他们不需要了解 Cosmos SDK 中 `x/distribution` 模块的具体实现细节。
相反，他们可以使用他们熟悉的以太坊接口与分发函数进行交互。

### 接口 `Distribution.sol`

在 [evmos/extensions 仓库中找到 Solidity 接口](https://github.com/evmos/extensions/blob/main/precompiles/stateful/Distribution.sol)。

### ABI

在 [evmos/extensions 仓库中找到 ABI](https://github.com/evmos/extensions/blob/main/precompiles/abi/distribution.json)。
