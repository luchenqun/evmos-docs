...
该文件定义了Tendermint的JSON-RPC规范。所有客户端都应该实现该规范。
...
## 支持
  ...
  | 路由                                    | [Tendermint-Go](https://github.com/tendermint/tendermint/) | [Tendermint-Rs](https://github.com/informalsystems/tendermint-rs) |
  |-----------------------------------------|:----------------------------------------------------------:|:-----------------------------------------------------------------:|
  | [健康状态](#health)                       |                             ✅                              |                                 ✅                                 |
  | [状态](#status)                       |                             ✅                              |                                 ✅                                 |
  | [网络信息](#netinfo)                     |                             ✅                              |                                 ✅                                 |
  | [区块链](#blockchain)               |                             ✅                              |                                 ✅                                 |
  | [区块](#block)                         |                             ✅                              |                                 ✅                                 |
  | [根据哈希获取区块](#blockbyhash)             |                             ✅                              |                                 ❌                                 |
  | [区块结果](#blockresults)           |                             ✅                              |                                 ✅                                 |
  | [提交](#commit)                       |                             ✅                              |                                 ✅                                 |
  | [验证人](#validators)               |                             ✅                              |                                 ✅                                 |
  | [创世块](#genesis)                     |                             ✅                              |                                 ✅                                 |
  | [分块创世块](#genesischunked)       |                             ✅                              |                                 ❌                                 |
  | [共识参数](#consensusparams)     |                             ✅                              |                                 ❌                                 |
  | [未确认交易](#unconfirmedtxs)       |                             ✅                              |                                 ❌                                 |
  | [未确认交易数量](#numunconfirmedtxs) |                             ✅                              |                                 ❌                                 |
  | [交易](#tx)                               |                             ✅                              |                                 ❌                                 |
  | [同步广播交易](#broadcasttxsync)     |                             ✅                              |                                 ✅                                 |
  | [异步广播交易](#broadcasttxasync)   |                             ✅                              |                                 ✅                                 |
  | [ABCI信息](#abciinfo)                   |                             ✅                              |                                 ✅                                 |
  | [ABCI查询](#abciquery)                 |                             ✅                              |                                 ✅                                 |
  | [异步广播交易](#broadcasttxasync)   |                             ✅                              |                                 ✅                                 |
  | [广播证据](#broadcastevidence) |                             ✅                              |                                 ✅                                 |

## 时间戳

Tendermint的RPC层中的时间戳遵循RFC3339Nano格式。RFC3339Nano格式会去除秒字段的尾部零。

这意味着如果一个区块的时间戳为：`1985-04-12T23:20:50.5200000Z`，在RPC中返回的值将是`1985-04-12T23:20:50.52Z`。
...
获取Tendermint状态，包括节点信息、公钥、最新区块哈希、应用哈希、区块高度和时间。
...
- `minHeight (integer)`: 响应中要返回的最低区块
- `maxHeight (integer)`: 响应中要返回的最高区块
...
- `height (integer)`: 请求的区块高度。如果未指定高度，则使用最新区块。
...
- `height (integer)`: 包含结果的区块的高度。如果未指定高度，则使用最新的区块高度。
...
- `height (integer)`: 请求的提交所属的区块的高度。如果未设置高度，则返回最新的提交。
...
- `height (integer)`: 验证人存在的区块高度。如果未设置高度，则返回最新的提交。
- `page (integer)`:
- `per_page (integer)`:
...
获取链的创世块。如果响应很大，此操作将返回错误：请使用 `genesis_chunked`。
...
以块的形式获取创世文档，以支持更大文档的传输。

#### 参数

- `chunk` (整数): 您希望获取的块的索引号。这些ID是从0开始的。
...
### 广播交易同步（BroadCastTxSync）

返回来自CheckTx的响应。不等待DeliverTx结果。
...
### 广播交易异步（BroadCastTxAsync）

立即返回，没有响应。不等待CheckTx和DeliverTx结果。
...
- `path (string)`: 数据的路径。由应用程序定义。
- `data (string)`: 请求的数据
- `height (integer)`: 请求数据的高度。
- `prove (bool)`: 包括区块中交易包含的证明
