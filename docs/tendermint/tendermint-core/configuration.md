...
Tendermint Core可以通过`$TMHOME/config/config.toml`中的TOML文件进行配置。其中一些参数可以通过命令行标志进行覆盖。对于大多数用户来说，`##### main base configuration options #####`中的选项是用于修改的，而下面的配置选项则是为高级用户准备的。

## 选项

通过`tendermint init`创建的默认配置文件已经将所有参数设置为它们的默认值。它将类似于下面的文件，但请通过检查使用您安装的`tendermint`版本创建的`config.toml`文件进行确认：
...
如果在您的配置中将`create_empty_blocks`设置为`true`，则会每秒钟创建一个块（使用默认的共识参数）。您可以通过更改`timeout_commit`来调节块之间的延迟。例如，`timeout_commit = "10s"`应该会导致大约10秒的块间隔。

### create_empty_blocks = false

在此设置中，块是在接收到交易时创建的。

注意，在块H之后，Tendermint会创建我们称之为“proof block”的东西（仅当应用哈希发生变化时才会创建H+1）。这样做的原因是为了支持证明。如果您在块H中有一个将状态更改为X的交易，新的应用哈希将仅包含在块H+1中。如果在提交您的交易后，您想要为新状态（X）获取轻客户端证明，您需要提交新的块以便这样做，因为新的块具有状态X的新应用哈希。这就是为什么如果应用哈希发生变化，我们会创建一个新的（空）块的原因。否则，您将无法为新状态生成证明。

此外，如果将`create_empty_blocks_interval`设置为除默认值（`0`）以外的其他值，Tendermint将在每个`create_empty_blocks_interval`时间间隔内即使没有交易也会创建空块。例如，使用`create_empty_blocks = false`和`create_empty_blocks_interval = "30s"`，只有在有交易或在等待30秒而没有接收到任何交易时，Tendermint才会创建块。

## 共识超时解释

关于超时的各种信息可以在[在生产环境中运行](./running-in-production.md)中找到。

您还可以在规范中找到更详细的技术解释：[BFT共识的最新八卦](https://arxiv.org/abs/1807.04938)。
...
请注意，在成功的轮次中，我们绝对等待的唯一超时是 `timeout_commit`。

以下是超时的简要总结：

- `timeout_propose` = 在预投票 nil 之前等待提案块的时间
- `timeout_propose_delta` = 每轮 `timeout_propose` 增加的时间
- `timeout_prevote` = 在收到 +2/3 预投票（不是单个块或 nil）后等待的时间
- `timeout_prevote_delta` = 每轮 `timeout_prevote` 增加的时间
- `timeout_precommit` = 在收到 +2/3 预提交（不是单个块或 nil）后等待的时间
- `timeout_precommit_delta` = 每轮 `timeout_precommit` 增加的时间
- `timeout_commit` = 在提交一个块后，在开始新高度之前等待的时间（这给我们一个机会接收一些额外的预提交，即使我们已经有 +2/3）
